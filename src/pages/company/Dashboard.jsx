...
import { Briefcase, Users, UserCheck, UserPlus, Clock, PlusCircle, Bell, Eye, FileText } from "lucide-react";
    ...
    return (
      <div className="p-6 space-y-6 bg-[var(--bg-primary)] min-h-screen">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-[var(--bg-secondary)] p-4 rounded-lg animate-pulse h-24 border border-[var(--border-color)]" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="p-6 space-y-6 bg-[var(--bg-primary)] min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[var(--text-secondary)]">Welcome back, {data.company?.name || "Company"}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="bg-[var(--bg-secondary)] p-4 rounded-lg shadow-sm border border-[var(--border-color)]">
              ...
              {data.recentApplications?.length > 0 ? (
                data.recentApplications.map((app) => (
                  <tr key={app.id} className="border-t border-[var(--border-color)]">
                    ...
                    <td className="p-4 text-[var(--text-primary)]">{app.job_title}</td>
                    <td className="p-4 text-[var(--text-secondary)]">
                      {app.applied_at ? new Date(app.applied_at).toLocaleDateString() : "—"}
                    </td>
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${statusBadge(app.status)}`}>
                        {app.status}
                      </span>
                    </td>
                  </tr>
