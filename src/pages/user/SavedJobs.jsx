  ...
  useEffect(() => {
    const fetchSavedJobs = async () => {
      try {
        ...
        // Fetch job details for each saved ID
        const results = await Promise.all(
          savedIds.map(async (id) => {
            try {
              ...
              return null; // job may have been deleted
            }
          })
        );
      ...
      <main className="flex flex-col gap-4 max-w-4xl">
        {savedJobs.length === 0 ? (
          ...
          savedJobs.map((job) => (
            <div
              key={job.id}
              className="bg-[var(--bg-primary)] p-6 rounded-xl border border-[var(--border-color)] shadow-sm flex flex-col sm:flex-row justify-between gap-4 group"
            >
