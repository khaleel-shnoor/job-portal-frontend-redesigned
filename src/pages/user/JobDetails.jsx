...
export default function JobDetails() {
  ...
  useEffect(() => {
    ...
    api.get("/applications/my")
      ...
      .catch(() => {}); // silently ignore — don't block page render
  }, [job, user]);
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col gap-8">
          ...
          <section className="bg-[var(--bg-primary)] p-8 rounded-xl border border-[var(--border-color)] shadow-sm">
            ...
            <p className="text-[var(--text-primary)] leading-relaxed whitespace-pre-line">{job.description}</p>
          </section>
        </div>
            ...
            {user?.role === "admin" ? (
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={`w-full py-4 font-bold rounded-xl shadow-sm text-lg transition-colors border ${
                    job.is_taken_down
              ...
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => !hasApplied && !job.is_taken_down && job.status !== "closed" && navigate(`/user/apply/${job.id}`)}
                  disabled={hasApplied || job.is_taken_down || job.status === "closed"}
                  className={`w-full py-4 font-bold rounded-xl shadow-sm text-lg transition-colors ${
                    hasApplied
