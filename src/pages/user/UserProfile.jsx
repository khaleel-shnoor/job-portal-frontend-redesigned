...
export default function UserProfile() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfile({
        ...
        skills: Array.isArray(userData.skills) ? userData.skills : (userData.skills ? JSON.parse(userData.skills) : []),
        projects: Array.isArray(userData.projects) ? userData.projects : (userData.projects ? JSON.parse(userData.projects) : []),
      });
      setError("");
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-8 h-full max-w-4xl pb-12">
      ...
      {success && <div className="p-3 bg-green-100 text-green-700 rounded-xl">{success}</div>}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center">
            ...
            <input
              ...
              disabled={!isEditing}
              className="flex-1 p-3 rounded-r-xl bg-[var(--bg-primary)] border border-[var(--border-color)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-accent)] disabled:opacity-60 transition-colors"
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-[var(--bg-primary)] p-6 rounded-xl border border-[var(--border-color)] shadow-sm flex flex-col gap-4">
        ...
        />
        {isEditing && <p className="text-xs text-gray-500">Upload new resume (PDF, DOC, DOCX)</p>}
      </section>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {profile.skills.map(skill => (
            <span key={skill} className="px-3 py-1 bg-[var(--bg-secondary)] text-[var(--text-primary)] text-sm font-semibold rounded-full border border-[var(--border-color)] flex items-center gap-2">
              {skill}
              {isEditing && (
                <button onClick={() => setProfile({ ...profile, skills: profile.skills.filter(s => s !== skill) })} className="text-[var(--text-secondary)] hover:text-red-500 font-bold">&times;</button>
              )}
            </span>
          ))}
        </div>
        ...
        {profile.projects.map((project) => (
          <div key={project.id} className="flex flex-col md:flex-row gap-4 mb-2">
            <input
              ...
              disabled={!isEditing}
              className="flex-1 p-3 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-accent)] disabled:opacity-60 transition-colors"
            />
            <input
              type="url"
              ...
              disabled={!isEditing}
              className="flex-[2] p-3 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--color-accent)] disabled:opacity-60 transition-colors"
            />
            {isEditing && (
              <button onClick={() => setProfile({ ...profile, projects: profile.projects.filter(p => p.id !== project.id) })} className="text-red-500 hover:text-red-700 font-bold px-2">✕</button>
            )}
          </div>
        ))}
        {isEditing && (
