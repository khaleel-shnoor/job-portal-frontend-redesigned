  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      ...
      <header>
        ...
        <p className="text-[var(--text-secondary)] mt-1">Here's a snapshot of your job hunt activity.</p>
      </header>

      {/* ── Stat Cards ─────────────────────────────────────────── */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {statCards.map(({ label, value, color }) => (
          <div key={label} className="bg-[var(--bg-primary)] p-5 rounded-xl border border-[var(--border-color)] shadow-sm flex flex-col gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[var(--bg-primary)] rounded-xl border border-[var(--border-color)] shadow-sm p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/user/profile" className="text-xs font-semibold text-[var(--color-accent)] hover:underline">
              Edit →
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-[var(--color-primary)] flex items-center justify-center text-white text-2xl font-bold shrink-0">
              {(profile.name || user?.name || "?")[0].toUpperCase()}
            </div>
            <div>
              <p className="font-bold text-[var(--text-primary)] leading-tight">{profile.name || user?.name}</p>
              <p className="text-sm text-[var(--text-secondary)]">{profile.title || "No title set"}</p>
              <p className="text-xs text-[var(--text-secondary)] mt-0.5">{profile.email || user?.email}</p>
            </div>
          </div>

          {/* Bio */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-1">About</p>
          ...
          {/* Skills */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)] mb-2">Skills</p>
            {profile.skills?.length > 0 ? (
              <div className="flex flex-wrap gap-1.5">
                {profile.skills.slice(0, 6).map(s => (
                  <span key={s} className="px-2 py-0.5 bg-[var(--bg-secondary)] border border-[var(--border-color)] text-xs rounded-full text-[var(--text-primary)]">
                    {s}
                  </span>
                ))}
                {profile.skills.length > 6 && (
                  <span className="text-xs text-[var(--text-secondary)]">+{profile.skills.length - 6} more</span>
                )}
              </div>
            ) : (
          ...
          <div className="mt-auto pt-2 border-t border-[var(--border-color)]">
            {profile.resume_url ? (
              ...
              <Link to="/user/profile" className="text-sm text-[var(--text-secondary)] hover:text-[var(--color-accent)]">
                Upload resume →
              </Link>
            )}
          </div>
        ...
        <div className="lg:col-span-2 bg-[var(--bg-primary)] rounded-xl border border-[var(--border-color)] shadow-sm flex flex-col">
          <div className="p-5 border-b border-[var(--border-color)] flex items-center justify-between">
            ...
            <Link to="/user/applications" className="text-xs font-semibold text-[var(--color-accent)] hover:underline">
              View all →
            </Link>
          </div>
          ...
          {recentApps.length === 0 ? (
            ...
            <div className="divide-y divide-[var(--border-color)]">
              {recentApps.map(app => (
                <div key={app.id} className="p-4 flex items-center justify-between hover:bg-[var(--bg-secondary)] transition-colors">
                  <div>
                    <p className="font-semibold text-[var(--text-primary)] text-sm">{app.job_title}</p>
                    <p className="text-xs text-[var(--text-secondary)] mt-0.5">{app.company_name}</p>
                    <p className="text-xs text-[var(--text-secondary)] mt-0.5">
                      {app.applied_at ? new Date(app.applied_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : ""}
                    </p>
                  </div>
                  <span className={`text-xs font-bold px-3 py-1 rounded-full capitalize ${statusStyles[app.status] || "bg-gray-100 text-gray-600"}`}>
                    {app.status}
                  </span>
                </div>
              ))}
          ...
          <div className="flex flex-col gap-3">
            {statCards.slice(0, 4).map(({ label, value, bar }) => (
              <div key={label} className="flex items-center gap-4">
                <span className="w-28 text-sm text-[var(--text-secondary)] font-medium shrink-0">{label}</span>
                ...
                <span className="w-6 text-sm font-bold text-[var(--text-primary)] text-right">{value}</span>
              </div>
            ))}
          </div>
        </div>
