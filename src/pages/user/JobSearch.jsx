          ...
          <div className="bg-[var(--bg-primary)] p-6 rounded-xl border border-[var(--border-color)] shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              ...
              {jobTypeOptions.map((option) => (
                <label
                  key={option.value}
                  className="flex items-center gap-3 text-[var(--text-primary)] cursor-pointer hover:text-[var(--color-accent)] transition-colors"
                >
                  <input
                    type="checkbox"
            ...
            <Link to={`/user/jobs/${job.id}`} key={job.id} className="block group">
              <div className="bg-[var(--bg-primary)] p-6 rounded-xl border border-[var(--border-color)] shadow-sm group-hover:shadow-md group-hover:border-[var(--color-accent)] transition-all flex flex-col sm:flex-row justify-between gap-4">
                <div className="flex-1">
                  <h2 className="text-xl font-bold text-[var(--color-primary)] group-hover:text-[var(--color-accent)] transition-colors">
                    {job.title}
                  </h2>
                  ...
                  {job.salary_min && job.salary_max && (
                    <p className="text-sm text-[var(--text-secondary)] mt-1">
                      {new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(job.salary_min)}{" "}
                      –{" "}
                      {new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 0 }).format(job.salary_max)}
                    </p>
                  )}
                ...
                <div className="flex sm:flex-col justify-end items-end gap-2">
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      setSavedJobIds((prev) =>
                        prev.includes(job.id) ? prev.filter((sid) => sid !== job.id) : [...prev, job.id]
                      );
                    }}
